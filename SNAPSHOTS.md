...
The 7810 is worse than iteration8 **for snapshots**: `win7810` (<its LAN IP>) is
`<its Windows hostname>`, confirmed Windows 10 22H2, and it has no `rsync` at all (it is
the Git-for-Windows bundle, not full MSYS2, so no `pacman` either) — never mind
the hardlinks Timeshift's rsync mode relies on.

**That conclusion is right for snapshots and was wrong as a general dismissal**
(corrected 2026-08-09). It is reachable over **ssh as user `<its Windows user>`**, so SMB never
enters into it; iteration8 already holds a working key for it; and it has **1.6 T
free on `D:`** — more headroom than iteration8's entire RAID array. For flat
archive files, which need no hardlinks, no POSIX modes and no ownership, it is an
excellent second location. Note it does not answer ping: Windows Defender blocks
ICMP echo by default, so use the ARP entry or a TCP probe to port 22, not `ping`,
to decide whether it is up. Every
...
| | |
| --- | --- |
| `preflight.sh` | snapshot → push → rebuild in a VM → run the real upgrade → verdict. Run as yourself, never under sudo |
| `preflight.sh --confirm-good` | after applying and rebooting: check the machine really is good, then re-baseline |
| `vm-restore-test.sh restore` | unattended, ending in `golden.qcow2`; skips entirely when already built from the newest snapshot |
| `vm-restore-test.sh testbase` | + serial console, swap and the helpers, in a 5 MB overlay |
| `vm-restore-test.sh verify` | 21 checks, running the helpers' own `check`/`status` |
| `vm-restore-test.sh verify-control` | proves those checks can fail |
| `vm-restore-test.sh update-test` | runs the real upgrade in a disposable overlay |
| `vm-restore-test.sh usb-image` | a VM-approved image, bootable from USB, identifiers rewritten — **never run** |

**What the first real run found**, all of it in the "reports success it has not
earned" family: a green verdict on a no-op upgrade; phased updates declined
because a clone inherits the laptop's machine-id; a confident verdict about the
wrong kernel; an apt-hook check that read silence as failure; `grep -q` under
`pipefail` turning a successful match into a failed pipeline; a remote job whose
progress never displayed because the launching ssh never returned; and a verdict
describing a new kernel when none had arrived. Every one was found by a person
reading output, not by a check failing.

**Still unexercised:** `usb-image` entirely, and the push half of
`--confirm-good`.

**Current state:** local baseline `2026-08-09_21-36-46` (781,993 files, known
good, post-update); newest offsite `2026-08-09_18-42-05`, which is also what
`golden.qcow2` was built from, so the next `preflight --skip-push` skips the
restore.

## How it was left after the snapshot work (2026-08-08)
...
| Splitting the VM build across spindles — **tried 2026-08-10, does not work** | The restore's long phase reads the carrier and writes the target, so putting them on different disks should stop the head thrashing. It does not, because the premise is false on this host. Measured: **23m against a 20-21m baseline**, with `iostat` showing the devices taking turns rather than overlapping — `md0` sat at **~2%** for the entire restore phase while `sdc` did 37-82%. With **251G of RAM** the 19G carrier written in step 4 is still wholly in page cache when step 6 reads it back, so the reads never reach a platter and there is only ever one stream on disk: the writes. Moving those to RAID5 adds a parity penalty besides. **The lever that works here is writing less, not spreading it** — `mv` instead of convert saved 19G, and the marker skips 39G entirely. `MBA_VMTEST_CARRIER` remains configurable because a host with less RAM, where the carrier really would be evicted between phases, is a different machine |
...
| Jenni's server should be a flip of a switch | **Goal stated 2026-08-10.** `server-provision.sh` covers the machine side and is proven on a blank VM: packages, layout, redundancy detection, `apply`. What is *not* a switch-flip yet is the client side. `.offsite.conf` and `.home-backup.conf` are hand-written per site, so somebody has to know the host, the user, the paths and the repo name. Generating them from two or three questions is the missing piece. **Three things stay manual on purpose and should not be automated away**: the ssh key exchange (an access decision), the restic passphrase (hers, and it must never pass through this project), and tailscale membership. Note the per-user design already suits this — separate repos with separate passphrases mean neither of them can browse the other's backup, which matters more between people who trust each other than between people who do not. See [[jenni-macbookair]] |
| Bootable rescue disk for these machines — raised 2026-08-10, not started | **Most of it already exists; the gap is smaller than the OEM image's was.** A stock Mint live USB is already a rescue environment that boots on a MacBookAir6,x *with Wi-Fi*, because Mint ships `broadcom-sta` — which on a machine with **no ethernet** is the whole ballgame, and is exactly why Ubuntu live media is the wrong base. What it lacks is everything site-specific: the repo's scripts, `restic` to pull a backup, the vaulted camera firmware, and the restore procedure itself. **Note this is a different artefact from the OEM image, and conflating them would be a mistake.** The OEM image says "here is a working machine" and its first boot CONSUMES its wizard; a rescue disk says "your machine is broken" and must be reusable, read-only toward the internal disk, and safe to boot a hundred times. **Cheapest credible build:** Mint live ISO written with a second partition carrying `/opt/macbookair2014`, a static `restic`, `iteration8:~/archive/facetimehd/firmware.bin`, and a `RESCUE.md`. The restore logic is already written and tested — `guest-restore.sh` inside `vm-restore-test.sh` does a real snapshot restore, and `snapshot-offsite.sh disk-plan` already prints the `mkfs` commands that recreate the original UUIDs on a replacement disk. **The one genuinely new problem is credentials**: pulling a restic backup needs the passphrase, and a rescue disk that carries it is a rescue disk that hands your backups to whoever finds it. It should prompt, never store — the same reasoning that keeps the passphrase out of this repo. Related: [[restored-clone-identity]] — a rescue disk that restores a snapshot onto a live machine creates a second copy of that machine's tailnet identity |
| OEM image: Mint + these fixes, bootable, no install required | **Scripted 2026-08-10 as `oem-image.sh`; the one interactive step is in progress.** The ISO turned out to carry an `OEM install (for manufacturers)` boot entry passing `oem-config/enable=true`, so Mint arms its own first-boot wizard and none of it had to be reconstructed. The other design question -- how to script the phases after the installer without putting ssh and somebody's key into a shipped image -- is answered by adding a serial console OFFLINE over nbd (`wire`), which `seal` removes again, so the image gains no package it would not otherwise have. Original scoping follows. The mechanism for the provisioning half of the two-golden thread below. Goal: hand someone a USB they boot into a working Mac-on-Linux with a "create your account" wizard — no installing, macOS untouched, stick out and they are back. **Use OEM mode, not an ISO remaster**: `oem-config`/`oem-config-gtk` are in Mint's archive (`24.04.3+mint19`), which is the supported way to ship a preconfigured system. Build: install Mint fresh in a VM (NOT from Joe's snapshot — see the rule about never restoring it onto someone else's machine), apply the fixes, `oem-config-prepare`, shut down, and that qcow2 is the image. **Most of the machinery already exists** — `usb-image`'s UUID rewriting, `nbd_attach`, the chroot + `update-grub` pass, and `qemu-img convert -O raw`. Genuinely new: the one-off fresh install, and a `provision.sh` that orchestrates the existing `install` subcommands (`mba-webcam.sh install`, `kbd-backlight.sh install`, `kernel-guard.sh install-hook`) — which do not currently have a single entry point or a documented order. **Two things OEM mode does not cover:** `/etc/machine-id` and the SSH host keys must be blanked so each first boot generates its own, or every stick and every refurbished machine shares one identity — the tailnet problem at fleet scale, see [[restored-clone-identity]]. **Size: build ~20 GiB and grow on first boot.** A fresh Mint 22.3 XFCE install is roughly 9G (estimate — this laptop's 15.3G `/usr` is years of accumulation), plus ~0.5G of DKMS toolchain that must stay resident because it rebuilds `wl` and `facetimehd` on every kernel, plus a fallback kernel: call it 10-12G. 20 GiB leaves ~40% free, fits any 32 GB stick, and — with `growpart` + `resize2fs` on first boot, the trick cloud images use — the *same* image expands to fill a 128 GB internal SSD. One artefact for both the try-it-out stick and the refurbished machine, rather than two. The rebuilt disk size is a knob (`MBA_VMTEST_TARGET_GB`); note 26 GiB fits a 32 GB stick but leaves the root ~77% full. That is fine for a try-it-out stick and mean for a daily driver — an OEM image someone actually lives on wants 32-40 GiB and a bigger stick, or the internal disk of the machine it is destined for. **`optimize-mba.sh` must be opt-in**: Joe declined the power tuning for his own machine and a provisioning image should not impose it. Note plain Mint already ships the Broadcom driver, so stock media usually gets Wi-Fi on a MacBookAir6,1 — what it lacks is the camera, the backlight fix and the kernel guard |
...
| ~~Move the offsite copy onto redundant storage~~ — **DONE 2026-08-10** | The store is now on the RAID5. `/srv/mba-snapshots` is a symlink to `/home/mba-snapshots` (on `md0`), so `.offsite.conf`, `vm-restore-test.sh` and the rsync daemon config all keep working unchanged — the path stayed canonical rather than N configs being updated and one being missed. Verified from both ends before the original was deleted: on iteration8 by entry count, byte count, hardlink count and the `%stat` xattr; from the laptop by `snapshot-offsite.sh verify`, whose subsequent `push` transferred **0 bytes** (`xfr#0`), confirming the single differing path had been a directory mtime rather than any file. **The first attempt failed** — see the `rsync -X` trap above, which produced a copy matching on entries, bytes and hardlinks with every ownership xattr missing. `sdc` went from 11% to 9% used; `/home` sits at 89% after reclaiming 149G of root reserve with `tune2fs -m 1`. Original context follows. | **Found 2026-08-09.** `/srv/mba-snapshots` — the laptop's entire disaster-recovery copy, 22G — lives on **`/dev/sdc4`, the one disk in iteration8 with no redundancy.** The other three (`sda`, `sdb`, `sdd`) are a RAID5 `md0` holding `/home`; `sdc` is standalone and carries iteration8's root, the snapshots, and the VM images. So the copy that exists *because a disk might die* is itself a single point of failure. Raised after a recent drive failure, which is precisely the scenario it insures against. **`sdc` itself is healthy** — SMART PASSED, 0 reallocated, 0 pending, 0 offline uncorrectable, 3536 power-on hours — so this is not urgent, it is structural. The fix is to move the 22G onto `md0`, which has 287G free, at the cost of taking `/home` from 92% to ~95%, and repoint `snapshot-offsite.sh`. **Only the snapshots need moving**: `golden.qcow2` and the overlays are rebuildable from them in twenty minutes and can stay on the non-redundant disk. Worth knowing while considering this: **writes do not wear these disks.** Write endurance is an SSD concern; an HDD's wear is mechanical — spindle hours and head actuation — and accrues whether or not anything is being written. The ~40G a restore writes costs essentially nothing in disk life, so the I/O volume is the wrong thing to economise on. Redundancy is the right one |
| The u810 recovery media is vaulted — 2026-08-09, DONE | The most irreplaceable data on this laptop turned out to be in `~/Downloads`: **8.2G `fujitsu-u-810-recovery-disk-1`** (vendor recovery media for a 2007 machine that Fujitsu no longer serves), 2.2G `u810-recovery`, and a 1.3G 32-bit `boron` ISO. 11.7G, existing nowhere else, on a laptop. Now at `iteration8:~/archive/u810` on the **RAID5**, and verified independently — `rsync -n --checksum` over both ends, **0 differing entries**. Deliberately NOT copied: the batocera game packages, which already sit in `~/backups/mba-home/batocera-backups` on the same array *and* are reproducible from the source discs in `~/discs`. A third copy on the 7810 is easy and not yet done — i8 can already authenticate as `<its Windows user>`, `D:` has 1.6T free, and `tar` over ssh works since it has no rsync |
